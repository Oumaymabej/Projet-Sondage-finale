  ...
  if (validateInput() == "") {
    ...
    if (!is.null(inFile)) {
      ...
      if (file_ext(inFile$name) %in% c("xlsx")) {
        validate(need(input$sheetName, "Veuillez sélectionner une feuille Excel."))
        uploadData$fiData <- read.xlsx2(
...
output$hasMissing <- reactive({
  if (!is.null(uploadData$fiData)) {
    return(any(colSums(is.na(uploadData$fiData)) > 0))
  }
  return(FALSE)
})
outputOptions(output, 'hasMissing', suspendWhenHidden = FALSE)
...
output$missing_histogram <- renderHighchart({
  req(uploadData$fiData)
  
  total_df <- missing_values_count(uploadData$fiData)
  
  # Filter only columns with missing values
  total_df <- total_df[total_df$count > 0, ]
  
  if (nrow(total_df) == 0) {
    # No missing values
    highchart() %>%
      hc_title(text = "Aucune valeur manquante") %>%
    ...
    highchart() %>%
      ...
      hc_tooltip(pointFormat = "Colonne: <b>{point.category}</b><br>Valeurs manquantes: <b>{point.y}</b>")
  }
})
